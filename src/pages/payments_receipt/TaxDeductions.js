...
const columns = [
  ...
  { key: 2, name: "Codigo de impuesto", fieldName: "TaxCodeId", minWidth: 150, maxWidth: 200, isResizable: true },
  { key: 3, name: "Descripción", fieldName: "Description", minWidth: 150, maxWidth: 200, isResizable: true },
  { key: 4, name: "Monto de deducción", fieldName: "AmountInTransactionCurrency", minWidth: 150, maxWidth: 200, isResizable: true },
  { key: 5, name: "Incluido en factura", fieldName: "TaxCode", minWidth: 150, maxWidth: 200, isResizable: true },
]
...
export const TaxDeductions = ({ PayStatementNumber }) => {
  ...
  const dataFiltered = data?.filter((item) => item.EmployerContribution === "No").map((tax) => {
    const data = {
      ...tax,
      AmountInTransactionCurrency: MontoFormateado(tax.AmountInTransactionCurrency)
    }
    return data;
  });
